...
int32_t read_term(int32_t fd, void* buf, int32_t nbytes) {
    ...
    memset(&(((char*)buf)[endline]), 0, nbytes - endline);
...
void get_prev_cmd(void){
    ...
    if(terminal->cmd_history[terminal->find_his_counter].cmd[0] == '\0'){
        if(terminal->find_his_counter==0) return;
        terminal->find_his_counter++;
        return;
    }
...
void get_next_cmd(void){
    int i;
    terminal->find_his_counter++;
    if(terminal->find_his_counter>=100) terminal->find_his_counter=terminal->find_his_counter%100;
    if(terminal->cmd_history[terminal->find_his_counter].cmd[0] == '\0'){
        if(terminal->find_his_counter==0) return;
        terminal->find_his_counter--;
        if(terminal->buffer[0]=='\0') return;
        for(i = 0; i < terminal->buffer_count; i++){
            putc_by_keyboard('\b');
        }
