...
// 
void change_ATTRIB_FOR_COVER(uint8_t attrib, int x, int y){
    change_term_vidmap_same();
...
void change_ATTRIB_FOR_COVER_BACK(){
    change_term_vidmap_same();
    int32_t i;
    for (i = 0; i < NUM_ROWS * NUM_COLS; i++) {
        *(uint8_t *)(video_mem + (i << 1) + 1) = term_color[visible_term];
...
/*
* build_hwcontext
*   DESCRIPTION: build the hardware context for the signal handler
*   INPUTS: ret_addr_ptr -- the pointer to the return address
*           signum_ptr -- the pointer to the signal number
*           hwcontext -- the pointer to the hardware context
*           ret_handler_size -- the size of the return handler
*   OUTPUTS: none
*   RETURN VALUE: none
*   SIDE EFFECTS: none
*/
void build_hwcontext(uint32_t* ret_addr_ptr, int32_t* signum_ptr, hardware_context_t* hwcontext, uint32_t ret_handler_size){
    //copy the return to the user stack
    memcpy((void*)((*ret_addr_ptr)-sizeof(hardware_context_t)-8), (void*)(ret_addr_ptr), 4);
    //copy the signal number to the user stack
    memcpy((void*)((*ret_addr_ptr)-sizeof(hardware_context_t)-4), signum_ptr, 4);
    //copy the hardware context to the user stack
    memcpy((void*)((*ret_addr_ptr)-sizeof(hardware_context_t)), hwcontext, sizeof(hardware_context_t));
    //copy the handler to the user stack
    memcpy((void*)(*ret_addr_ptr), link_handler_sig_return, ret_handler_size); 
}
